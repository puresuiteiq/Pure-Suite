...
const NAME = 'add-storefront-background-dark'
...
async function main() {
  ...
  try {
    ...
    await addColumn(conn, 'storefront_background_dark')
    await addColumn(conn, 'storefront_background_shadow_dark')
    await conn.query('INSERT INTO applied_migrations (name) VALUES (?)', [NAME])
    console.log('Storefront dark-mode background columns added.')
  } finally {
    conn.release()
    await pool.end()
  }
