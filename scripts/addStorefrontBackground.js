...
const NAME = 'add-storefront-background'
...
async function main() {
  ...
  try {
    ...
    await addColumn(conn, 'storefront_background')
    await addColumn(conn, 'storefront_background_shadow')
    await conn.query('INSERT INTO applied_migrations (name) VALUES (?)', [NAME])
    console.log('Storefront background columns added.')
  } finally {
    conn.release()
    await pool.end()
  }
