...
async function addColumn(conn, column) {
  ...
  if (rows[0].n) return
  await conn.query(`ALTER TABLE merchants ADD COLUMN ${column} DECIMAL(10,7) NULL`)
}
...
async function main() {
  ...
  try {
    ...
    await addColumn(conn, 'latitude')
    await addColumn(conn, 'longitude')
    await conn.query('INSERT INTO applied_migrations (name) VALUES (?)', [NAME])
    console.log('Location (latitude/longitude) columns added.')
  } finally {
    conn.release()
    await pool.end()
  }
